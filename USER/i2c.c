 ...
 * @LastEditors: xw.qu
 * @LastEditTime: 2023-08-31 10:53:41
 * @FilePath: \USER\i2c.c
 * @Description: i2c RTC
 * 
...
void rdtime(void)
{
	...
	// 星期可以不用处理
	rtcdata[7] = 0;
	write_dgusii_vp(0x0010, (u8 *)rtcdata, 8); // 写入DGUS变量空间
}
...
//	btn_check[2]=x;
//	btn_check[3]=y;
//	write_dgusii_vp(0x00D4,(u8*)&btn_check[0],4);
// }
...
void check_rtc_set(void)
{
	...
	if ((rtc_parm[0] == 0x5A) && (rtc_parm[1] == 0xA5)) // 启动配置
	{
		...
		rtc_parm[0] = 0;
		rtc_parm[1] = 0;
		write_dgusii_vp(0x9c, rtc_parm, 2);
	}
}
